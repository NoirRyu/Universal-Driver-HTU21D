    ...
    )
{
    ...
    InitPropVariantFromString(L"MDS Technology", 
    ...
    InitPropVariantFromString(L"Environment Custom sensor V2", 
	...
	m_pSupportedDataFields->List[CSTM_DATA_TEMPERATURE] = PKEY_SensorData_Temperature_Celsius;
	m_pSupportedDataFields->List[CSTM_DATA_HUMIDITY] = PKEY_SensorData_RelativeHumidity_Percent;
	...
	m_pData->List[CSTM_DATA_TEMPERATURE].Key = PKEY_SensorData_Temperature_Celsius;
	InitPropVariantFromFloat(0.0f, &(m_pData->List[CSTM_DATA_TEMPERATURE].Value));
	m_pData->List[CSTM_DATA_HUMIDITY].Key = PKEY_SensorData_RelativeHumidity_Percent;
	InitPropVariantFromFloat(0.0f, &(m_pData->List[CSTM_DATA_HUMIDITY].Value));
...
//chris 
NTSTATUS
CustomSensorDevice::ConfigureIoTarget(
	_In_ WDFCMRESLIST ResourcesRaw,         // Supplies a handle to a collection of framework resource
											// objects. This collection identifies the raw (bus-relative) hardware
											// resources that have been assigned to the device.
	_In_ WDFCMRESLIST ResourcesTranslated)  // Supplies a handle to a collection of framework
											...
											// device. The resources appear from the CPU's point of view.
{
	...
	{
		m_I2CIoTarget = NULL;
		Status = WdfIoTargetCreate(m_FxDevice, WDF_NO_OBJECT_ATTRIBUTES, &m_I2CIoTarget);
