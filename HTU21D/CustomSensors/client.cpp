...
{    
	...
	BYTE HumDataBuffer[3];

	WdfWaitLockAcquire(m_Lock, NULL);
	Status = I2CSensorReadRegister(m_I2CIoTarget, SAMPLE_TEMPERATURE_HOLD, &TempDataBuffer[0], sizeof(TempDataBuffer));
	WdfWaitLockRelease(m_Lock);

	WdfWaitLockAcquire(m_Lock, NULL);
	Status = I2CSensorReadRegister(m_I2CIoTarget, SAMPLE_HUMIDITY_HOLD, &HumDataBuffer[0], sizeof(HumDataBuffer));
	WdfWaitLockRelease(m_Lock);


	//To get Temperature 
	unsigned short rawTemperature = static_cast<unsigned short>(TempDataBuffer[0] << 8);
	rawTemperature |= static_cast<unsigned short>(TempDataBuffer[1] & 0xFC);

	float tempTemperature = rawTemperature / static_cast<float>(65536);
	float realTemperature = -46.85 + (175.72 * tempTemperature); 

	//To get Humidity 
	unsigned short rawHumidity = static_cast<unsigned short>(HumDataBuffer[0] << 8);
	rawHumidity |= static_cast<unsigned short>(HumDataBuffer[1] & 0xFC);

	float tempHumidity = rawHumidity / static_cast<float>(65536);	
	float realHumidity = -6 + (125 * tempHumidity);
	
	//configure sensor data field
	InitPropVariantFromFloat(realTemperature, &(m_pData->List[CSTM_DATA_TEMPERATURE].Value));
	InitPropVariantFromFloat(realHumidity,    &(m_pData->List[CSTM_DATA_HUMIDITY].Value));
    ...
    )
{
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        //chris  WdfWaitLockAcquire(pDevice->m_Lock, NULL);
        ...
        //chris WdfWaitLockRelease(pDevice->m_Lock);
...
)
{
    ...
    *pSize = 0;
    ...
    //chris if ((*DataField == pDevice->m_pSupportedDataFields->List[CSTM_DATA_CO2_LEVEL_PERCENT]))
	if ((*DataField == pDevice->m_pSupportedDataFields->List[CSTM_DATA_TEMPERATURE])   &&
		(*DataField == pDevice->m_pSupportedDataFields->List[CSTM_DATA_HUMIDITY]))
    {
        if (nullptr == pProperties)
        {
            // Just return size
