...
namespace CustomSensors
{
    public sealed partial class Scenario1_DataEvents : Page
    {
        ...
        //chris const String CO2LevelKey = "{74879888-a3cc-45c6-9ea9-058838256433} 1";
        const String Temperature = "{c458f8a7-4ae8-4777-9607-2e9bdd65110a} 101";
        const String Humidity = "{c458f8a7-4ae8-4777-9607-2e9bdd65110a} 102";
        ...
        private async void OnCustomSensorAdded(DeviceWatcher watcher, DeviceInformation customSensorDevice)
        {
            try
            {
                ...
                if (customSensor != null)
                {
                    ...
                    if (!reading.Properties.ContainsKey(Temperature) &&
                        !reading.Properties.ContainsKey(Humidity))
                    {
                        rootPage.NotifyUser("The found custom sensor doesn't provide Environment reading", NotifyType.ErrorMessage);
                        customSensor = null;
                    }
                    else
                    {
        ...
        protected override void OnNavigatingFrom(NavigatingCancelEventArgs e)
        {
            ...
            {
                Window.Current.VisibilityChanged -= new WindowVisibilityChangedEventHandler(OnVisibilityChanged);

                if (customSensor != null)
                {
                    customSensor.ReadingChanged -= new TypedEventHandler<CustomSensor, CustomSensorReadingChangedEventArgs>(OnReadingChanged);
        ...
        {
            if (customSensor != null && ScenarioDisableButton.IsEnabled)
            {
                if (e.Visible)
                {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private async void OnReadingChanged(object sender, CustomSensorReadingChangedEventArgs e)
        {
            CustomSensorReading reading = e.Reading;

            string TempLevelString = String.Format("{0,5:0.00}", reading.Properties[Temperature]);
            string HumidityLevelString = String.Format("{0,5:0.00}", reading.Properties[Humidity]);
            await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
            {
                ScenarioOutputTempLevel.Text = TempLevelString;
                ScenarioOutputHumidityLevel.Text = HumidityLevelString;
            });
        }
